          ...
          {result.recommendations.map((recommendation, index) => (
            <article className="rounded-3xl border border-[var(--line)] bg-white p-5 sm:p-6" key={recommendation.id}>
              ...
              <dl className="mt-5 grid grid-cols-2 gap-4 rounded-2xl bg-[var(--surface-subtle)] p-4 text-sm">
                <div><dt className="text-xs font-semibold text-[var(--text-muted)]">{recommendation.verification === "verified" ? "Published minimum" : "Illustrative score"}</dt><dd className="mt-1 font-extrabold text-[var(--brand-950)]">{recommendation.minimumScore}+</dd></div>
              ...
              {(() => {
                const source = academicDataset.sources.find(
                  (item) => item.id === recommendation.sourceId,
                );
                ...
                return (
                  ...
                  >
                    View {source.verification === "verified" ? "official" : "demo"} source
