...
import { PortableOpportunityLab } from "@/components/app/portable-opportunity-lab";
  ...
  return (
    <>
      ...
      <section className="page-shell py-10 sm:py-14">
        <PageHeading eyebrow="College comparison" title="Same credits, different maps" description="Compare today’s totals, test one future exam, and trace every credit through UF, FIU, and UCF using reviewed official records." />
          ...
          {comparisons.map((comparison) => {
            ...
            <article className={`rounded-3xl border bg-white p-5 sm:p-6 ${comparison.university.id === plan.universityId ? "border-[var(--brand-500)] shadow-[0_16px_35px_rgba(21,55,65,0.08)]" : "border-[var(--line)]"}`} key={comparison.university.id}>
              <div className="flex items-start justify-between gap-4">
                <div><div className="flex flex-wrap items-center gap-2"><span className="text-xs font-extrabold tracking-[0.1em] text-[var(--brand-700)] uppercase">{comparison.university.shortName}</span>{comparison.university.id === plan.universityId && <span className="rounded-full bg-[var(--mint-50)] px-2 py-1 text-[0.65rem] font-bold text-[var(--brand-700)] uppercase">Current plan</span>}<span className={`rounded-full px-2 py-1 text-[0.65rem] font-bold uppercase ${source?.verification === "verified" ? "bg-emerald-50 text-emerald-700" : "bg-[var(--warning-soft)] text-[var(--warning-strong)]"}`}>{source?.verification === "verified" ? "Verified sources" : "Demo data"}</span></div><h2 className="mt-1 text-xl font-extrabold tracking-[-0.03em] text-[var(--brand-950)]">{comparison.university.name}</h2><p className="mt-1 text-xs text-[var(--text-muted)]">{comparison.program.name}</p></div>
